...
class CAVEngine:
    ...
    def _check_columns(self, stmt) -> str:
        ...
        for col in stmt.find_all(exp.Column):
            ...
            if table_ref:
                ...
                allowed_cols = {
                    c.lower() for c in self.schema["tables"][real_table]["columns"]
                }
                ...
                for t in tables_in_query:
                    if t in self.schema["tables"]:
                        allowed = {
                            c.lower() for c in self.schema["tables"][t]["columns"]
                        }
    ...
    def _is_valid_join(
        self, l_table: str, l_col: str, r_table: str, r_col: str
    ) -> bool:
        ...
        l_pks = [
            p.lower()
            for p in self.schema["tables"].get(l_table, {}).get("primary_keys", [])
        ]
        r_pks = [
            p.lower()
            for p in self.schema["tables"].get(r_table, {}).get("primary_keys", [])
        ]
