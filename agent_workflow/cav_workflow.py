"""
Phase 3: Automated Correction Loop (LangGraph)
Nodes: Generator → CAV Verifier → Executor
Conditional routing: if CAV fails, feed error back to Generator (max 3 retries).
"""
...
from sql_agent.cav_engine.cav_engine import CAVEngine
...
class CAVWorkflow:
    def __init__(
        ...
        schema_path: str,
        ollama_model: str = "hf.co/defog/sqlcoder-7b-2:Q5_K_M",
    ):
        self.engine = engine
    ...
    # Node: Generator
    # ------------------------------------------------------------------ #
    def _make_prompt(self, question: str, error_msg: str = "", previous_sql: str = "") -> str:
        # 1. Read the external prompt file
        prompt_path = "/home/abhinav/Documents/AI Engineering/SQL_Agent/sql_agent/Config/cav-sql.md"
    ...
    def _executor_node(self, state: CAVState) -> CAVState:
        ...
        try:
            ...
            result_json = json.dumps({"columns": cols, "rows": rows[:50]}, indent=2, default=str)
    ...
    def _route_after_verifier(self, state: CAVState) -> Literal["executor", "generator", "end_max_retries"]:
